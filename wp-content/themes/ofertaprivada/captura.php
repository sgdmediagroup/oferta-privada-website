...
-----END CERTIFICATE-----");
$configuration->setWebpayCert("-----BEGIN CERTIFICATE-----
...
if ($_GET && $_GET["authorizationCode"] && $_GET["captureAmount"] && $_GET["buyOrder"]) {
...
if ($_GET && $_GET["token"]) {
	$token = $_GET["token"];
	$result = $webpay->getCaptureTransaction()->getTransactionResult($token);
	echo json_encode($result);
}